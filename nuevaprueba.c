...
void receiveFile(int socket, const char *filePath, const char *listaArchivosPath, const char *localDir) {
    ...
    printf("Nombre del archivo: %s y extensión %s\n", fileName, fileExtension);

    struct FileInfo fileInfo = getFileInfo(listaArchivosPath, fileName);
    if (strcmp(fileInfo.action, "No encontrado") == 0) {
        printf("El archivo %s no se encontró en el historial.\n", fileName);
    } else {
        printf("Fecha Archivo entrante: %sFecha archivo lista: %s", ctime(&modificationTime), ctime(&fileInfo.lastModified));
        if (difftime(fileInfo.lastModified, fileInfo.lastModified) > 0){
            printf("El archivo entrante es el más nuevo\n");
            //strcat(fileName, "New");
            //renameFileInHistory(filePath, fileName, strcat(fileName, "Old"), localDir);
        } else if (difftime(modificationTime, fileInfo.lastModified) < 0) {
            printf("el archivo de la lista es mas nuevo\n");
            //strcat(fileName, "Old");
        ...
        /*printf("Nombre: %s\n", fileInfo.filename);
        printf("Tamaño: %ld bytes\n", fileInfo.size);
        printf("Fecha de Modificación: %s", ctime(&fileInfo.lastModified));
        printf("Acción: %s\n\n", fileInfo.action);*/
    }
